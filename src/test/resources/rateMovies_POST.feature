...
Feature: POST: Rate movies Validations
  ...
  Scenario: Validate status of movie
    ...
    * print sessionId
    ...
    Then status 201
    * def status = response
    * match status == { "success": true, "status_code": 12, "status_message": '#string' }
    ...
    * match status == { "status_code": 7, "status_message": "Invalid API key: You must be granted a valid key.", "success": false }
    ...
    Then status 401
    * def status = response
    * match status == { "success": false, "status_code": 3, "status_message": "Authentication failed: You do not have permissions to access the service." }
